...
public class FaceOverlayView extends View {
    ...
    private void initialize() {
        ...
        int stroke = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 3, metrics);
        mPaint = new Paint();
            ...
            for (FaceResult face : mFaces) {
                ...
                if(isDrawLive) {
                    if (face.getLive() == 0) {
                        ...
                        BoxTextPaint.setColor(Color.RED);
                        canvas.drawText("Fake", rectF.left, rectF.top, BoxTextPaint);
                    } else if(face.getLive() == 1) {
                        mPaint.setColor(Color.GREEN);
                        BoxTextPaint.setColor(Color.GREEN);
                        canvas.drawText("Real", rectF.left, rectF.top, BoxTextPaint);
                    }else if(face.getLive() == 2){
                        mPaint.setColor(Color.YELLOW);
                        BoxTextPaint.setColor(Color.YELLOW);
                        canvas.drawText("N/A", rectF.left, rectF.top, BoxTextPaint);
                    }
                }
...
//                canvas.drawText("EyesDistance " + face.eyesDistance(), rectF.left, rectF.bottom + mTextPaint.getTextSize() * 3, mTextPaint);
    ...
    public static void drawLandMarks (Bitmap image, List <FaceLandmark> facePoints){
        Canvas canvas = new Canvas(image);
        Paint paint = new Paint();
        paint.setColor(Color.RED);
        paint.setStyle(Paint.Style.STROKE);//不填充
        paint.setStrokeWidth(5);  //线的宽度
        for (FaceLandmark fp : facePoints) {
            int left, top, right, bottom;
            //画特征点
            int pointCount = fp.getLandmarks2D().length;
            float[] drawPoint = new float[pointCount];

            for (int i=0; i<pointCount; i++)
            {
                drawPoint[i] = (float)fp.getLandmarks2D()[i];
            }
            canvas.drawPoints(drawPoint, paint);//画多个点
        }
    }
